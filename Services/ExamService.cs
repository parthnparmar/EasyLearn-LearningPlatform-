...
public class ExamService : IExamService
{
    ...
    public ExamService(ApplicationDbContext context, ICertificateService certificateService, IAchievementService achievementService, IEmailService emailService)
    {
        _context = context;
    ...
    public async Task<ExamAttempt?> StartExamAsync(string studentId, int examId)
    {
        ...
        // Send exam started email
        var student = await _context.Users.FindAsync(studentId);
        var examForEmail = await _context.Exams.Include(e => e.Course).FirstOrDefaultAsync(e => e.Id == examId);
        if (student?.Email != null && examForEmail != null)
        {
            var studentName = $"{student.FirstName} {student.LastName}".Trim();
            _ = _emailService.SendEmailAsync(
                student.Email,
                $"Exam Started: {examForEmail.Title}",
                $"<h2>Your Exam Has Started!</h2><p>Hi {studentName},</p><p>Your exam <strong>{examForEmail.Title}</strong> for <strong>{examForEmail.Course.Title}</strong> is now active.</p><p>You have <strong>{examForEmail.DurationMinutes} minutes</strong> to complete it. Good luck!</p><p>EasyLearn Team</p>"
            );
        }
    ...
    public async Task<bool> SubmitPartAAsync(int examAttemptId, List<ExamAnswerSubmission> answers)
    {
        try
        {
            ...
            // Get Part A questions (25 MCQs × 2 marks = 50 marks)
            ...
            // Clear existing answers for this attempt and part (student-specific isolation)
            ...
            // Process all Part A questions - AUTOMATIC SCORING (2 marks per correct MCQ)
            foreach (var question in partAQuestions)
            {
                ...
                if (answer?.SelectedOptionId != null)
                {
                    ...
                    if (isCorrect)
                    {
                        points = 2; // Fixed: 2 marks per correct MCQ
                    }
                ...
                // Store student-specific answer (isolated per examAttemptId)
                var examAnswer = new ExamAnswer
                {
                    ExamAttemptId = examAttemptId, // Ensures answer isolation per student
            ...
            attempt.PartAScore = partAScore; // Max 50 marks (25 × 2)
        ...
        {
            Console.WriteLine($"Error in SubmitPartAAsync: {ex.Message}");
            return false;
        }
    }
    ...
    public async Task<bool> SubmitPartBAsync(int examAttemptId, List<ExamAnswerSubmission> answers)
    {
        ...
        // Store student-specific Part B answers (isolated per examAttemptId)
        foreach (var answer in answers)
        {
            ...
            if (existingAnswer != null)
            {
                ...
                existingAnswer.Points = 0; // Will be graded by instructor (30 marks theory)
