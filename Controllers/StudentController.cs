...
public class StudentController : Controller
{
    ...
    public async Task<IActionResult> EnrollDirect(PaymentViewModel model)
    {
        ...
        if (!string.IsNullOrEmpty(model.StudentEmail))
        {
            await _emailService.SendEmailAsync(
                model.StudentEmail,
                $"Enrollment Confirmed: {course.Title}",
                $"<h2>Welcome to {course.Title}!</h2><p>Hi {model.StudentName},</p><p>You have successfully enrolled in <strong>{course.Title}</strong>. Start learning today!</p><p>Happy Learning,<br/>EasyLearn Team</p>"
            );
        }
    ...
    public async Task<IActionResult> SubmitQuiz(int quizId, IFormCollection form)
    {
        ...
        var quizAttempt = new QuizAttempt
        {
            ...
            CompletedAt = DateTime.UtcNow,
            TotalPoints = quiz.Questions.Sum(q => q.Points) // Sum of custom points
        };
        ...
        foreach (var question in quiz.Questions)
        {
            ...
            if (selectedAnswerIds.Length > 0 && int.TryParse(selectedAnswerIds[0], out int answerId))
            {
                ...
                var studentAnswer = new StudentAnswer
                {
                    ...
                    AnswerText = "",
                    IsCorrect = isCorrect,
                    Points = isCorrect ? question.Points : 0 // Use question's custom points
                };
                ...
                {
                    correctAnswers++;
                    totalScore += question.Points; // Use custom points
                }
            }
            // If no answer selected, don't create StudentAnswer (unattempted = 0 points)
        }

        // Calculate percentage based on custom scoring system
        var maxPossibleScore = quiz.Questions.Sum(q => q.Points); // Sum of all question points
        quizAttempt.Score = totalScore;
        quizAttempt.Percentage = maxPossibleScore > 0 ? (totalScore * 100.0 / maxPossibleScore) : 0;
        quizAttempt.IsPassed = quizAttempt.Percentage >= quiz.PassingScore;
    ...
    public async Task<IActionResult> TakeExam(int examId)
    {
        ...
        // Get student's specific exam schedule
        ...
        ViewBag.StudentSchedule = schedule; // Pass student's specific schedule
    ...
    public async Task<IActionResult> ExamInterface(int examId)
    {
            ...
            if (completedAttempt != null)
            {
                ...
                if (hasPaidReExam)
                {
                    ...
                    attempt = await _context.ExamAttempts
                        .Include(ea => ea.Exam)
                        .ThenInclude(e => e.Course)
                        .FirstOrDefaultAsync(ea => ea.Id == attempt.Id);
                }
                else
                {
        ...
        // ONLY redirect to Part B if Part A is completed
        if (attempt!.PartACompleted && !attempt.PartBCompleted)
        {
            // Check if Part B questions exist
            var partBCount = await _context.ExamQuestions
                .CountAsync(eq => eq.ExamId == examId && eq.Part == ExamPart.PartB);
            
            if (partBCount > 0)
            {
                return RedirectToAction("TakePartB", new { examId, attemptId = attempt.Id });
            }
        }
    ...
    public async Task<IActionResult> SubmitExamPartA(int examId, int attemptId, IFormCollection form)
    {
        ...
        // Check if exam has Part B questions
        var hasPartB = attempt.Exam.ExamQuestions.Any(q => q.Part == ExamPart.PartB);
        ...
        {
            // No Part B, complete the exam
            attempt.PartBCompleted = true;
            attempt.IsCompleted = true;
            attempt.CompletedAt = DateTime.UtcNow;
            await _context.SaveChangesAsync();
            
            TempData["Success"] = "Exam completed successfully!";
    ...
    public async Task<IActionResult> TakePartB(int examId, int attemptId)
    {
        ...
        // CRITICAL: Verify this attempt belongs to the current student
        ...
        // Check if there are Part B questions
        if (!exam.ExamQuestions.Any())
        {
            // No Part B questions, complete the exam
            attempt.PartBCompleted = true;
            attempt.IsCompleted = true;
            attempt.CompletedAt = DateTime.UtcNow;
            await _context.SaveChangesAsync();
            
            TempData["Info"] = "This exam has no Part B questions. Exam completed.";
            return RedirectToAction("ExamResult", new { attemptId });
        }
    ...
    public async Task<IActionResult> PYQ()
    {
        ...
        var pyqs = await _context.PYQs
            .Include(p => p.Course)
            .ThenInclude(c => c.Category)
            .Include(p => p.Exam)
            .Where(p => enrolledCourseIds.Contains(p.CourseId) && p.IsActive)
    ...
    [Route("pyq/{pyqId:int}")]
    public async Task<IActionResult> ViewPYQ(int pyqId)
    {
        var studentId = _userManager.GetUserId(User)!;
        
        var pyq = await _context.PYQs
            .Include(p => p.Course)
        ...
        if (pyq == null)
        {
            TempData["Error"] = "PYQ not found.";
            return RedirectToAction("PYQ");
        }
        
        var isEnrolled = await _context.Enrollments
            .AnyAsync(e => e.StudentId == studentId && e.CourseId == pyq.CourseId);
        
        if (!isEnrolled)
        {
            TempData["Error"] = "You must be enrolled in this course to view PYQ.";
            return RedirectToAction("BrowseCourses");
        }
    ...
    [HttpGet]
    [Route("download-pyq/{pyqId:int}")]
    public async Task<IActionResult> DownloadPYQ(int pyqId)
    {
        ViewBag.DownloadUrl = Url.Action("DownloadPYQDirect", new { pyqId });
        ViewBag.FileName = "PYQ paper";
        ViewBag.ReturnUrl = Request.Headers["Referer"].ToString() ?? Url.Action("PYQ");
        return View("DownloadStatus");
    }

    [Route("download-pyq-direct/{pyqId:int}")]
    public async Task<IActionResult> DownloadPYQDirect(int pyqId)
    {
        var studentId = _userManager.GetUserId(User)!;
        ...
        if (!isEnrolled)
        {
            TempData["Error"] = "You must be enrolled in this course to download PYQ.";
            return RedirectToAction("BrowseCourses");
        }
        ...
        html.AppendLine(".question { margin: 20px 0; page-break-inside: avoid; }");
        html.AppendLine(".option { margin-left: 20px; }");
        html.AppendLine("</style></head><body>");
        
        html.AppendLine($"<div class='header'>");
        html.AppendLine($"<h1>{pyq.Course.Title}</h1>");
        html.AppendLine($"<h2>Previous Year Question Paper</h2>");
        html.AppendLine($"<p>Exam Date: {pyq.ExamDate:MMM dd, yyyy}</p>");
        html.AppendLine($"<p>Total Questions: {pyq.Questions.Count}</p>");
        html.AppendLine($"</div>");
        
        for (int i = 0; i < pyq.Questions.Count; i++)
        {
            var question = pyq.Questions.ElementAt(i);
            html.AppendLine($"<div class='question'>");
            html.AppendLine($"<p><strong>Q{i + 1}. {question.Text}</strong> ({question.Points} marks)</p>");
            
            if (question.Options?.Any() == true)
            {
                foreach (var option in question.Options.OrderBy(o => o.OrderIndex))
                {
                    html.AppendLine($"<p class='option'>{option.Text}</p>");
                }
            }
            html.AppendLine($"</div>");
        }
        ...
        var fileName = $"PYQ_{pyq.Course.Title.Replace(" ", "_")}_{pyq.ExamDate:yyyyMMdd}.html";
    ...
    public async Task<IActionResult> ProcessCertificatePayment(CertificatePaymentViewModel model)
    {
        try
        {
            ...
            if (!string.IsNullOrEmpty(model.StudentEmail))
            {
                await _emailService.SendEmailAsync(
                    model.StudentEmail,
                    $"Payment Confirmed: {enrollment.Course.Title}",
                    $"<h2>Payment Successful!</h2><p>Hi {model.StudentName},</p><p>Your payment of ₹800 for <strong>{enrollment.Course.Title}</strong> has been received. You now have access to exams and your certificate.</p><p>Receipt No: {receipt.ReceiptNumber}</p><p>EasyLearn Team</p>"
                );
            }
    ...
    {
        ViewBag.DownloadUrl = Url.Action("DownloadCertificateReceiptDirect", new { receiptId });
        ViewBag.FileName = "receipt";
        ViewBag.ReturnUrl = Request.Headers["Referer"].ToString() ?? Url.Action("MyCourses");
        return View("DownloadStatus");
    }

    [Route("download-certificate-receipt-direct/{receiptId:int}")]
    public async Task<IActionResult> DownloadCertificateReceiptDirect(int receiptId)
    {
        try
        {
            var studentId = _userManager.GetUserId(User)!;
    ...
    {
        ViewBag.DownloadUrl = Url.Action("DownloadCertificateDirect", new { courseId });
        ViewBag.FileName = "certificate";
        ViewBag.ReturnUrl = Request.Headers["Referer"].ToString() ?? Url.Action("MyCourses");
        return View("DownloadStatus");
    }

    [Route("download-certificate-direct/{courseId:int}")]
    public async Task<IActionResult> DownloadCertificateDirect(int courseId)
    {
        try
        {
            var studentId = _userManager.GetUserId(User)!;
            ...
            // Send certificate download notification email
            ...
            if (!string.IsNullOrEmpty(student?.Email))
            {
                ...
                await _emailService.SendEmailAsync(
                    student.Email,
                    $"Certificate Downloaded: {enrollment.Course.Title}",
                    $"<h2>Certificate Downloaded!</h2><p>Hi {studentName},</p><p>Your certificate for <strong>{enrollment.Course.Title}</strong> (Certificate No: {certificate.CertificateNumber}) has been downloaded successfully.</p><p>Congratulations on completing the course!</p><p>EasyLearn Team</p>"
                );
            }
    ...
    {
        ViewBag.DownloadUrl = Url.Action("DownloadExamCertificateDirect", new { attemptId });
        ViewBag.FileName = "exam certificate";
        ViewBag.ReturnUrl = Request.Headers["Referer"].ToString() ?? Url.Action("ExamResults");
        return View("DownloadStatus");
    }

    [Route("download-exam-certificate-direct/{attemptId:int}")]
    public async Task<IActionResult> DownloadExamCertificateDirect(int attemptId)
    {
        try
        {
            var studentId = _userManager.GetUserId(User)!;
    ...
    {
        ViewBag.DownloadUrl = Url.Action("ExportProgressDirect");
        ViewBag.FileName = "progress report";
        ViewBag.ReturnUrl = Request.Headers["Referer"].ToString() ?? Url.Action("MyCourses");
        return View("DownloadStatus");
    }

    [Route("export-progress-direct")]
    public async Task<IActionResult> ExportProgressDirect()
    {
        try
        {
            var studentId = _userManager.GetUserId(User)!;
